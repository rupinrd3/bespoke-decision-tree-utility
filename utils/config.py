...
DEFAULT_CONFIG = {
    ...
    "missing_values": {
        "strategy": "ignore",  # Options: "ignore", "remove_rows", "impute", "raise"
    ...
    "decision_tree": {
        ...
        "split_finding": {
            "max_features_to_evaluate": 30,
            "backend": "threading",
            "n_jobs": -2,
            "max_threads": 4,
            "max_splits_returned": 50
        }
    },
    ...
    "metrics": {
        ...
        "threshold": 0.05,  # Decision threshold for binary classification (optimized for low-base-rate risk models)
        "positive_class": None,  # None = second class by default
        "parallel_probability": {
            "probability_backend": "loky",
            "min_samples_for_parallel": 1000,
            "memory_threshold_gb": 8.0,
            "fallback_backend": "sequential"
        }
    },
...
def validate_configuration(config: Dict[str, Any]) -> bool:
    ...
    valid_strategies = ['ignore', 'remove_rows', 'impute', 'raise', 'none']
    strategy = missing_config.get('strategy')
    if strategy not in valid_strategies:
        logger.warning(
            f"Invalid missing value strategy: {strategy}, using 'ignore' instead"
        )
        missing_config['strategy'] = 'ignore'
        valid = False
