...
class BespokeDecisionTree(QObject):  # Re-enable QObject inheritance for signals
    ...
    def _validate_inputs(self, X: pd.DataFrame, y: pd.Series, 
        ...
        if X.isna().any().any():
            ...
            missing_strategy = self.config.get('missing_values', {}).get('strategy', 'ignore')
            ...
            elif missing_strategy in ('ignore', 'none'):
                logger.info("Leaving missing values in place for split-time handling (strategy=%s)", missing_strategy)
            else:
    ...
    def _initialize_root_node(self, X: pd.DataFrame, y: pd.Series, sample_weight: Optional[np.ndarray] = None):
        ...
        try:
            ...
            impurity = self._calculate_impurity(
                {cls: float(count) for cls, count in class_counts.items()}
            )
    ...
    def _initialize_root_node_with_data(self, X, y):
        ...
        try:
            ...
            if class_counts:
                self.root.prediction = majority_class
    ...
    def _find_best_split(self, X: pd.DataFrame, y: pd.Series, sample_weight: np.ndarray, 
        ...
        backend = split_config.get('backend') or ('threading' if split_config.get('use_threading', True) else 'sequential')
        requested_n_jobs = split_config.get('n_jobs', -2 if backend in ('threading', 'loky') else 1)

        if backend == 'threading':
            n_jobs = requested_n_jobs if requested_n_jobs != -2 else normalise_n_jobs(-2)
            if requested_n_jobs > 0:
                n_jobs = normalise_n_jobs(requested_n_jobs)
        elif backend == 'loky':
            n_jobs = requested_n_jobs
        else:
            n_jobs = 1

        X_block = X.iloc[indices]
        y_block = y.iloc[indices]
        weights_block = sample_weight[indices] if sample_weight is not None else np.ones(len(indices))

        tasks: List[SplitTask] = []
        for feature in features_to_consider:
            try:
                if X_block[feature].isna().all():
                    continue
                feature_values = X_block[feature].to_numpy(copy=False)
                task = SplitTask(
                    feature=feature,
                    feature_values=feature_values,
                    target_values=y_block.to_numpy(copy=False),
                    weights=np.asarray(weights_block, dtype=float),
                    current_impurity=node.impurity,
                    feature_type='categorical' if feature in self.categorical_features else 'numeric',
                    target_classes=self.target_values or [],
                    min_samples_leaf=self.min_samples_leaf,
                    criterion=self.criterion.value if isinstance(self.criterion, SplitCriterion) else str(self.criterion),
        ...
        try:
            if backend == 'loky' and tasks:
                results = Parallel(n_jobs=n_jobs, backend='loky', batch_size=1)(
                    delayed(evaluate_split)(t) for t in tasks
                )
            elif backend == 'threading' and tasks:
            ...
            for res in results:
                if res is None:
                    continue
                if best_result is None or res[1].get('gain', -float('inf')) > best_result[1].get('gain', -float('inf')):
                    best_result = res

        except Exception as exc:
            logger.warning(f"Parallel split evaluation failed in training; falling back to legacy: {exc}")
            best_result = None
            ...
            for feature in features_to_consider:
                ...
                if feature in self.categorical_features:
                    split_found, split_info = self._find_best_categorical_split(
                        X, y, sample_weight, feature, indices, current_impurity
                    )
                else:
        ...
        split_info = best_result[1]
        if split_info.get('gain', -float('inf')) > self.min_impurity_decrease:
    ...
    def _get_node_sample_indices(self, X: pd.DataFrame, node: TreeNode) -> List[int]:
        ...
        for parent, child in path:
            ...
            if parent.split_type == 'numeric':
                missing_child_idx = getattr(parent, 'missing_value_child_idx', 0)
                valid_mask = ~pd.isna(feature_values)
                missing_mask = ~valid_mask
                
                if not valid_mask.any():
                    if child_idx != missing_child_idx:
                ...
                if child_idx == 0:  # Left child (<=)
                    mask = (valid_mask & (feature_values <= threshold)) | (missing_mask & (missing_child_idx == 0))
                else:  # Right child (>)
                    mask = (valid_mask & (feature_values > threshold)) | (missing_mask & (missing_child_idx == 1))
                ...
                if hasattr(parent, 'split_thresholds') and parent.split_thresholds:
                    ...
                    if child_idx == 0:
                        mask = valid_mask & (feature_values < thresholds[0])
                ...
                normalized_values = feature_values.apply(
                ...
                if hasattr(parent, 'split_categories') and parent.split_categories:
                    normalized_map = {
                        ("Missing" if pd.isna(cat) else str(cat)): idx
                        for cat, idx in parent.split_categories.items()
                    }
                    mapped = normalized_values.map(normalized_map)
                    mask = mapped == child_idx
                    mask = mask | (mapped.isna() & (child_idx == missing_child_idx))
                else:
                    if child_idx == 0 and hasattr(parent, 'left_categories'):
                        normalized_left = {
                            "Missing" if pd.isna(cat) else str(cat)
                            for cat in parent.left_categories
                        }
                        mask = normalized_values.isin(normalized_left)
                    elif child_idx == 1 and hasattr(parent, 'right_categories'):
                        normalized_right = {
                            "Missing" if pd.isna(cat) else str(cat)
                            for cat in parent.right_categories
                        }
                        mask = normalized_values.isin(normalized_right)
                    else:
                        mask = pd.Series(child_idx == missing_child_idx, index=normalized_values.index)
                    
                    if child_idx == missing_child_idx:
                        mask = mask | normalized_values.eq("Missing")
                
                indices = indices[mask.values]

        return indices.tolist()

    def _build_numeric_split_masks(self, feature_series: pd.Series, threshold: float,
                                   operator: str = '<=') -> Tuple[pd.Series, pd.Series]:
        """
        Build boolean masks for numeric splits honoring the requested operator.
        ...
        Returns:
            Tuple of (left_mask, right_mask) boolean Series
        """
        operator = operator or '<='
        if operator == '<':
            left_mask = feature_series < threshold
        elif operator == '>':
            left_mask = feature_series > threshold
        elif operator == '>=':
            left_mask = feature_series >= threshold
            ...
            if operator != '<=':
                logger.warning(f"Unsupported split operator '{operator}', defaulting to '<='")
            left_mask = feature_series <= threshold

        left_mask = left_mask.fillna(False)
        missing_mask = feature_series.isna()
        right_mask = (~left_mask) | missing_mask
        return left_mask, right_mask

    def _resolve_node_sample_indices(self, node: TreeNode) -> Optional[List[int]]:
        """
        Ensure node.sample_indices are positional, in-bounds, and usable.

        Falls back to recalculating from the current tree structure when cached
        indices are missing or invalid. This prevents stale indices from crashing
        manual split operations (especially on fresh builds with non-binary targets).
        """
        if not hasattr(self, '_cached_X') or self._cached_X is None:
            logger.error("No cached dataset available to resolve sample indices")
            return None

        total_rows = len(self._cached_X)
        raw_indices = getattr(node, 'sample_indices', None)

        if not raw_indices:
            logger.info(f"Node {node.node_id} missing sample indices, recalculating from tree path")
            return self._get_node_sample_indices(self._cached_X, node)
        ...
        try:
            for idx in raw_indices:
                if isinstance(idx, (np.integer, int)):
                    ...
                    logger.warning(
                        f"Node {node.node_id} has non-integer sample index '{idx}', recalculating"
                    )
                    return self._get_node_sample_indices(self._cached_X, node)

                if idx_int < 0 or idx_int >= total_rows:
                    logger.warning(
                        f"Node {node.node_id} sample index {idx_int} out of bounds (size={total_rows}), recalculating"
                    )
                    return self._get_node_sample_indices(self._cached_X, node)
            ...
            logger.warning(
                f"Failed to validate cached sample indices for node {node.node_id}: {exc}, recalculating"
            )
            return self._get_node_sample_indices(self._cached_X, node)

        if not cleaned_indices:
            logger.warning(f"Node {node.node_id} sample indices empty after validation, recalculating")
            return self._get_node_sample_indices(self._cached_X, node)
    ...
    def _find_best_numerical_split(self, X: pd.DataFrame, y: pd.Series, 
        ...
        total_samples_considered = len(indices)
        valid_sample_count = len(feature_values)
        sample_coverage = (
        ...
        for target_value, weight in zip(target_values, weights):
            parent_counts[target_value] += float(weight)
        parent_impurity = self._calculate_impurity(parent_counts)
        ...
        if best_split_idx >= 0:
            left_weight_total = sum(best_split_info.get('left_counts', {}).values())
            right_weight_total = sum(best_split_info.get('right_counts', {}).values())
            best_split_info['samples_evaluated'] = valid_sample_count
            best_split_info['sample_coverage'] = sample_coverage
            best_split_info['total_samples_considered'] = total_samples_considered
            best_split_info['missing_value_child_idx'] = 0 if left_weight_total >= right_weight_total else 1
    ...
    def _find_best_categorical_split(self, X: pd.DataFrame, y: pd.Series, 
        ...
        if len(categories) > 10:
            split_found, split_info = self._find_best_grouped_categorical_split(
                feature, categories, feature_values, target_values, weights, parent_impurity
            )
        else:
            split_found, split_info = self._find_best_binary_categorical_split(
                feature, categories, feature_values, target_values, weights, parent_impurity
            )
        
        if split_found and split_info is not None:
            left_weight_total = sum(split_info.get('left_counts', {}).values())
            right_weight_total = sum(split_info.get('right_counts', {}).values())
            split_info['samples_evaluated'] = valid_sample_count
            split_info['sample_coverage'] = sample_coverage
            split_info['total_samples_considered'] = total_samples_considered
            split_info['missing_value_child_idx'] = 0 if left_weight_total >= right_weight_total else 1
    ...
    def _find_best_grouped_categorical_split(self, feature: str, categories: np.ndarray,
                                          ...
                                          weights: np.ndarray, parent_impurity: float) -> Tuple[bool, Dict[str, Any]]:
        """
        ...
        Args:
            ...
            parent_impurity: Impurity for valid samples of this feature
    ...
    def _apply_split(self, X: pd.DataFrame, split_info: Dict[str, Any]) -> Tuple[List[int], List[int]]:
        ...
        if split_info['split_type'] == 'numeric':
            ...
            missing_child_idx = split_info.get('missing_value_child_idx', 0)
            if missing_child_idx == 0:
                left_mask = np.logical_or(left_mask, missing_mask)
            else:
                right_mask = np.logical_or(right_mask, missing_mask)
            
            left_indices = [i for i in range(len(feature_values)) if left_mask[i]]
            right_indices = [i for i in range(len(feature_values)) if right_mask[i]]
            ...
            in_left = feature_series.isin(left_categories).to_numpy()
            not_left = ~in_left
            missing_child_idx = split_info.get('missing_value_child_idx', 0)
            
            if missing_child_idx == 0:
                in_left = np.logical_or(in_left, missing_mask)
                right_mask = np.logical_and(not_left, ~missing_mask)
            else:
                right_mask = np.logical_or(np.logical_and(not_left, ~missing_mask), missing_mask)
            
            left_indices = [i for i in range(len(in_left)) if in_left[i]]
            right_indices = [i for i in range(len(right_mask)) if right_mask[i]]
    ...
    def _create_children_from_split(self, X: pd.DataFrame, y: pd.Series, sample_weight: np.ndarray,
        ...
        node.missing_value_child_idx = split_info.get('missing_value_child_idx', 0)
    ...
    def _apply_split_with_transaction(self, node: TreeNode, config: SplitConfiguration,
        ...
        try:
            ...
            sample_indices = self._resolve_node_sample_indices(node)
            if not sample_indices:
                logger.error(f"Could not determine sample indices for node {node.node_id}")
                return False

            node.sample_indices = sample_indices  # keep sanitized version for downstream consumers
    ...
    def _apply_numerical_split_enhanced(self, node: TreeNode, config: SplitConfiguration,
        ...
        try:
            ...
            operator = getattr(config, 'operator', getattr(config, 'split_operator', '<='))
            ...
            left_mask, right_mask = self._build_numeric_split_masks(feature_data, threshold, operator)
            ...
            if len(left_y) > 0:
                self._update_leaf_statistics(left_child, left_y)

            if len(right_y) > 0:
                self._update_leaf_statistics(right_child, right_y)
            ...
            node.split_rule = f"{feature} {operator} {threshold}"
    ...
    def _apply_categorical_split_enhanced(self, node, node_X, node_y, config, transaction) -> bool:
        ...
        try:
            ...
            feature_series = node_X[feature]
            normalized_series = feature_series.apply(
                lambda value: "Missing" if pd.isna(value) else str(value)
            )
            normalized_left = { "Missing" if pd.isna(cat) else str(cat) for cat in left_categories }
            
            left_mask = normalized_series.isin(normalized_left)
            ...
            if hasattr(node, 'sample_indices'):
                ...
                left_indices = normalized_series.index[left_mask].tolist()
                right_indices = normalized_series.index[right_mask].tolist()
            ...
            node.split_feature = feature
            all_categories = { "Missing" if pd.isna(val) else str(val) for val in normalized_series.unique() }
            right_categories = list(all_categories - normalized_left)
            
            split_categories = {}
            for cat in normalized_left:
            ...
            node.left_categories = list(normalized_left)
            node.right_categories = right_categories
            node.split_type = 'categorical'
            node.missing_value_child_idx = split_categories.get("Missing", 0)
            node.children = [left_child, right_child]
            node.is_terminal = False
    ...
    def _apply_categorical_multi_bin_split_enhanced(self, node, node_X, node_y, config, transaction) -> bool:
        ...
        try:
            ...
            for category, bin_idx in category_mapping.items():
                normalized_category = "Missing" if pd.isna(category) else str(category)
                if bin_idx not in bins:
                    bins[bin_idx] = []
                bins[bin_idx].append(normalized_category)
            ...
            for bin_idx in sorted(bins.keys()):
                ...
                mask = normalized_series.isin(categories)
                ...
                for category in categories:
                    split_categories[category] = len(child_indices_list) - 1
            ...
            node.split_feature = feature
            node.split_bin_categories = [bins[idx] for idx in sorted(bins.keys())]
            node.split_categories = split_categories
            node.split_type = 'categorical_multi_bin'
            node.children = child_nodes
            node.is_terminal = False
    ...
    def _update_leaf_statistics(self, node: TreeNode, y_values: pd.Series):
        """Populate standard leaf statistics (class counts, majority class, probability, impurity)."""
        try:
            if y_values is None or len(y_values) == 0:
                node.class_counts = {}
                node.samples = 0
                node.majority_class = None
                node.prediction = None
                node.probability = 0.0
                node.impurity = 0.0
                return
            
            class_counts = y_values.value_counts().to_dict()
            node.class_counts = class_counts
            node.samples = int(len(y_values))
            ...
            node.probability = (
                class_counts.get(majority_class, 0) / node.samples
                if node.samples > 0 else 0.0
            )
            node.impurity = self._calculate_node_impurity(y_values)
        except Exception as exc:
            logger.warning(
                f"Failed to update leaf statistics for node {getattr(node, 'node_id', '?')}: {exc}"
            )
            node.samples = int(len(y_values)) if y_values is not None else 0
            fallback_class = None
            if hasattr(node, 'parent') and getattr(node.parent, 'majority_class', None) is not None:
                fallback_class = node.parent.majority_class
            elif getattr(self, 'positive_class', None) is not None:
                fallback_class = self.positive_class
            elif getattr(self, 'target_values', None):
                fallback_class = self.target_values[0]
            
            node.majority_class = fallback_class
            node.prediction = fallback_class
            node.probability = 1.0 if fallback_class is not None and node.samples > 0 else 0.0
            node.class_counts = node.class_counts if hasattr(node, 'class_counts') else {}
            node.impurity = 0.0
        ...
        try:
            ...
            if hasattr(self, '_cached_X') and hasattr(self, '_cached_y') and sample_indices:
                try:
                    ...
                    self._update_leaf_statistics(child, child_y)
                    logger.debug(
                        f"Created child {child_id}: {child.samples} samples, impurity={child.impurity:.4f}"
                    )
                except Exception as e:
                    logger.warning(f"Could not calculate statistics for child {child_id}: {e}")
                    self._update_leaf_statistics(child, pd.Series(dtype=self._cached_y.dtype))
            else:
                if hasattr(parent, 'majority_class') and parent.majority_class is not None:
                    fallback_series = pd.Series([parent.majority_class] * len(sample_indices))
                    self._update_leaf_statistics(child, fallback_series)
                elif hasattr(self, 'target_values') and self.target_values:
                    fallback_series = pd.Series([self.target_values[0]] * len(sample_indices))
                    self._update_leaf_statistics(child, fallback_series)
                else:
                    self._update_leaf_statistics(child, pd.Series(dtype='object'))
    ...
    def _apply_categorical_multi_bin_split(self, node: TreeNode, feature: str,
        ...
        try:
            normalized_feature = feature_data.apply(
                lambda value: "Missing" if pd.isna(value) else str(value)
            )
            child_nodes = []
            ...
            for i, bin_config in enumerate(bin_configs):
                ...
                bin_mask = normalized_feature.isin(bin_categories)
                ...
                if len(bin_y) > 0:
                    self._update_leaf_statistics(child_node, bin_y)
                
                child_nodes.append(child_node)
                all_assigned_indices.update(bin_indices)
                for category in bin_categories:
                    category_map[category] = len(child_nodes) - 1
                ...
                if unassigned_indices:
                    ...
                    if child_nodes:
                        ...
                        if len(unassigned_y) > 0:
                            all_bin_y = node_y[node_y.index.isin(largest_bin.sample_indices)]
                            self._update_leaf_statistics(largest_bin, all_bin_y)
                            for category in feature_data[feature_data.index.isin(unassigned_indices)].unique():
                                category_map[category] = child_nodes.index(largest_bin)
            ...
            node.split_feature = feature
            node.split_type = 'categorical_multi_bin'
            node.multi_bin_category_map = category_map
            node.split_categories = category_map
            node.missing_value_child_idx = category_map.get("Missing", 0)
    ...
    def _calculate_node_impurity(self, y: pd.Series) -> float:
        """Calculate node impurity using the currently configured criterion"""
        if len(y) == 0:
            return 0.0
        return self._calculate_impurity(
            {cls: float(count) for cls, count in y.value_counts().items()}
        )
    ...
    def _select_features_for_split_optimized(self, X: pd.DataFrame, y: pd.Series, 
                                           indices: np.ndarray, max_features: Optional[int] = None) -> List[str]:
        """
        ...
        if max_features is None or len(all_features) <= max_features:
            return all_features
    ...
    def find_split_for_node(self, node_id: str, X: pd.DataFrame = None, y: pd.Series = None, 
        ...
        node_targets = y.iloc[indices]
        if sample_weight is not None:
            node_weights = sample_weight[indices]
            class_counts = defaultdict(float)
            for target_value, weight in zip(node_targets, node_weights):
                class_counts[target_value] += float(weight)
        else:
            class_counts = {cls: float(count) for cls, count in node_targets.value_counts().items()}
        current_impurity = self._calculate_impurity(class_counts)
        node.impurity = current_impurity
        
        split_config = self.config.get('decision_tree', {}).get('split_finding', {})
        if max_features is None:
            max_features = split_config.get('max_features_to_evaluate')
        if isinstance(max_features, int) and max_features <= 0:
            max_features = None
        
        backend = split_config.get('backend')
        if not backend:
            backend = 'threading' if split_config.get('use_threading', True) else 'sequential'
        ...
        requested_n_jobs = split_config.get(
            'n_jobs',
            -2 if backend in ('threading', 'loky') else 1
        )
        ...
        if backend == 'threading':
            ...
            if n_jobs == -2:
                ...
                n_jobs = normalise_n_jobs(n_jobs)
            else:
                n_jobs = 1
        elif backend == 'loky':
            n_jobs = requested_n_jobs
        ...
        def process_feature(feature):
            ...
            try:
                ...
                if split_found and split_info.get('gain', 0) > 0:
                    ...
                    coverage_msg = f", coverage={float(coverage):.2%}" if coverage is not None else ""
                    logger.debug(
                        "Feature %s: found split with gain=%.6f%s",
                        feature,
                        split_info.get('gain', 0.0),
                        coverage_msg
                    )
        ...
        valid_results: List[Optional[Tuple[str, Dict[str, Any]]]] = []

        if backend == 'loky' and features_to_consider:
            logger.info(
                "Evaluating splits with loky backend using %s jobs for %d features",
                n_jobs,
                len(features_to_consider),
            )
            X_block = X.iloc[indices]
            y_block = y.iloc[indices]
            weights_block = sample_weight[indices] if sample_weight is not None else np.ones(len(indices))
            target_array = y_block.to_numpy(copy=False)
            weights_array = np.asarray(weights_block, dtype=float)
            tasks: List[SplitTask] = []
            ...
            try:
                results = Parallel(
                    ...
                    backend='loky',
                    batch_size=1,
                )(delayed(evaluate_split)(task) for task in tasks)
            except Exception as exc:
                logger.error(
                    "loky split evaluation failed; reverting to threading backend: %s",
                    exc,
                    exc_info=True,
                )
                backend = 'threading'
                ...
                logger.info(
                    "Parallel split evaluation completed via loky. Valid splits: %d/%d",
                    len(valid_results),
                    len(features_to_consider),
                )

        if (backend != 'loky') or not valid_results:
            logger.info(
                "Evaluating splits with %s backend using n_jobs=%s",
                backend,
                n_jobs,
            )
            if backend == 'threading':
                ...
                valid_results = [
                    result for result in (process_feature(feature) for feature in features_to_consider)
                    if result is not None
                ]
            logger.info(
                "Split evaluation completed via %s backend. Valid splits: %d/%d",
                backend,
                len(valid_results),
                len(features_to_consider),
            )
    ...
    def _apply_multi_bin_split(self, node: TreeNode, config: SplitConfiguration,
        ...
        try:
            ...
            def _format_bin_label(min_val, max_val) -> str:
                """Format bin ranges for display while avoiding trailing zeros."""
                def _format_value(val):
                    try:
                        numeric_val = float(val)
                    except (TypeError, ValueError):
                        return str(val)
                    text = f"{numeric_val:.3f}"
                    if '.' in text:
                        text = text.rstrip('0').rstrip('.')
                    return text

                min_text = _format_value(min_val)
                max_text = _format_value(max_val)
            ...
            for i, (min_val, max_val) in enumerate(bins):
                ...
                bin_label = _format_bin_label(min_val, max_val)
                child_node.bin_display_label = bin_label
            ...
            if assigned_samples != total_node_samples:
                ...
                if unassigned_indices:
                    ...
                    if child_nodes:
                        largest_bin = max(child_nodes, key=lambda x: x.samples)
                        missing_child_idx = child_nodes.index(largest_bin)
                        largest_bin.sample_indices.extend(list(unassigned_indices))
            ...
            node.missing_value_child_idx = missing_child_idx
            node.split_bin_labels = bin_labels
            
            if hasattr(config, 'thresholds') and config.thresholds:
                node.split_thresholds = sorted(config.thresholds)
            elif hasattr(config, 'bins') and config.bins:
                derived_thresholds = [max_val for _, max_val in config.bins[:-1]]
                if derived_thresholds:
                    node.split_thresholds = sorted(derived_thresholds)
            ...
            if split_type == 'numeric':
                ...
                operator = split_info.get('split_operator', '<=')
                success = self._apply_numeric_split_compatible(node, feature, threshold, node_X, node_y, operator)
    ...
    def _apply_numeric_split_compatible(self, node: 'TreeNode', feature: str, threshold: float, 
        ...
        try:
            ...
            left_mask_raw, right_mask_raw = self._build_numeric_split_masks(feature_data, threshold, operator)
            left_mask = left_mask_raw & valid_mask
            right_mask = right_mask_raw & valid_mask
            ...
            logger.info(f"Successfully applied numeric split: {feature} {operator} {threshold}")
            return True
            ...
            for i, (bin_name, categories) in enumerate(split_categories.items()):
                if i < len(children):
                    ...
                    if len(bin_target) > 0:
                        ...
                        child_impurity = self._calculate_impurity(
                            {cls: float(count) for cls, count in bin_class_counts.items()}
                        )
