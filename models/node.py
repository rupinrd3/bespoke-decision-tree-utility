...
class TreeNode:
    ...
    def __init__(self, node_id: Optional[str] = None, 
        ...
        self.split_thresholds = []          # Threshold list for multi-bin numeric splits
        self.missing_value_child_idx = 0    # Which child receives missing values
        self.left_categories = []
        self.right_categories = []
    ...
    def update_stats(self, sample_count=None, class_counts=None, majority_class=None,
                     impurity=None, criterion=None, **kwargs):
        """
        ...
        Args:
            ...
            impurity: Optional impurity value calculated by the caller
            criterion: Optional splitting criterion name/enum for fallback impurity calculation
        ...
        try:
            ...
            computed_impurity = impurity
            if computed_impurity is None:
                if criterion is None and 'criterion' in kwargs:
                    criterion = kwargs['criterion']
                computed_impurity = self._compute_impurity_from_counts(class_counts, criterion)
            try:
                self.impurity = float(computed_impurity)
    ...
    @staticmethod
    def _compute_impurity_from_counts(class_counts, criterion=None):
        """
        Compute impurity from class counts using the requested criterion.
        Defaults to Gini when criterion is not provided or invalid.
        """
        if not class_counts:
            return 0.0
        ...
        proportions = [count / total for count in class_counts.values() if count > 0]
        if not proportions:
            return 0.0
        
        criterion_name = None
        if criterion is not None:
            criterion_name = getattr(criterion, 'value', criterion)
            if criterion_name:
                criterion_name = str(criterion_name).lower()
        
        if criterion_name in ('entropy', 'information_gain'):
            return -sum(p * np.log2(p) for p in proportions)
        if criterion_name == 'misclassification':
    ...
    def predict(self, X: pd.DataFrame) -> np.ndarray:
        ...
        if self.split_type == 'numeric':
            if self.split_feature in X.columns:
                feature_series = X[self.split_feature]
                left_mask = feature_series <= self.split_value
                right_mask = feature_series > self.split_value
                missing_mask = feature_series.isna()
                missing_child_idx = getattr(self, 'missing_value_child_idx', 0)
    ...
    def _find_bin_for_value(self, node: 'TreeNode', feature_value: float) -> int:
        ...
        if hasattr(node, 'split_thresholds') and node.split_thresholds:
            thresholds = sorted(node.split_thresholds)
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        node_dict = {
            ...
            'missing_value_child_idx': self.missing_value_child_idx,
    ...
    def from_dict(cls, node_dict: Dict[str, Any], parent: Optional['TreeNode'] = None) -> 'TreeNode':
        ...
        node.split_thresholds = node_dict.get('split_thresholds', [])
        node.missing_value_child_idx = node_dict.get('missing_value_child_idx', 0)
        node.left_categories = node_dict.get('left_categories', [])
        node.right_categories = node_dict.get('right_categories', [])
