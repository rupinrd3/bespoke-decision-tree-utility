...
class NumericMultiBinSplit(SplitConfiguration):
    ...
    def get_child_assignment(self, values: pd.Series) -> pd.Series:
        ...
        first_mask = values < first_threshold
        assignments.loc[first_mask] = 0

        for idx in range(1, len(sorted_thresholds)):
            lower = sorted_thresholds[idx - 1]
            upper = sorted_thresholds[idx]
            mask = (values >= lower) & (values < upper)
            assignments.loc[mask] = idx

        last_threshold = sorted_thresholds[-1]
        last_mask = values >= last_threshold
        assignments.loc[last_mask] = len(sorted_thresholds)
