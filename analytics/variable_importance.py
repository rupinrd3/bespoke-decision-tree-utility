...
[VariableImportance.__init__ -> Initializes the VariableImportance -> dependent functions are None]
[VariableImportance.compute_importance -> Computes feature importance using the specified method -> dependent functions are _compute_impurity_importance, _compute_permutation_importance, _compute_drop_column_importance, _compute_gain_importance]
[VariableImportance.calculate_gini_importance -> Calculates Gini-based feature importance -> dependent functions are _get_all_tree_nodes, _calculate_node_importance]
[VariableImportance._get_all_tree_nodes -> Gets all nodes in the tree using iterative traversal -> dependent functions are None]
[VariableImportance._calculate_node_importance -> Calculates importance for a single node and recursively for its children -> dependent functions are None]
[VariableImportance._compute_impurity_importance -> Computes impurity-based importance using the tree structure -> dependent functions are _get_all_tree_nodes]
[VariableImportance.calculate_permutation_importance -> Calculates permutation-based feature importance -> dependent functions are _calculate_accuracy]
[VariableImportance._compute_permutation_importance -> Internal method for permutation importance calculation -> dependent functions are _tree_score]
[VariableImportance._compute_drop_column_importance -> Computes drop-column importance -> dependent functions are _compute_permutation_importance]
[VariableImportance._compute_gain_importance -> Computes gain-based importance from the tree structure -> dependent functions are _get_all_tree_nodes]
[VariableImportance._calculate_accuracy -> Calculates model accuracy using the existing predict method -> dependent functions are None]
[VariableImportance._tree_score -> Calculates score (accuracy) for a tree using tree prediction -> dependent functions are _predict_single_sample]
[VariableImportance._predict_single_sample -> Predicts a single sample by traversing the tree -> dependent functions are None]
[VariableImportance.calculate_feature_interactions -> Calculates pairwise feature interactions -> dependent functions are calculate_gini_importance, get_top_features, _calculate_accuracy]
[VariableImportance.get_top_features -> Gets the top N most important features -> dependent functions are None]
[VariableImportance.analyze_importance -> Analyzes feature importance distribution and provides insights -> dependent functions are None]
[VariableImportance.export_importance_report -> Exports importance analysis to a text report -> dependent functions are None]
[VariableImportance.plot_importance -> Creates a feature importance plot -> dependent functions are None]
[InteractionImportance.__init__ -> Initializes the InteractionImportance -> dependent functions are None]
[InteractionImportance.compute_interactions -> Computes pairwise feature interactions -> dependent functions are _tree_score]
[InteractionImportance._tree_score -> Calculates score (accuracy) for a tree -> dependent functions are _predict_sample]
[InteractionImportance._predict_sample -> Predicts a single sample -> dependent functions are None]
[InteractionImportance.plot_interactions -> Creates a feature interaction plot -> dependent functions are None]
"""
