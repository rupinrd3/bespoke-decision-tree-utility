...
[NodeReportGenerator.__init__ -> Initializes the NodeReportGenerator -> dependent functions are None]
[NodeReportGenerator.generate_node_report -> Generates a report for all terminal nodes in a decision tree -> dependent functions are _validate_inputs, _extract_terminal_nodes, _create_node_number_mapping, _get_tree_level_statistics, _calculate_node_statistics, _validate_calculation_results, _calculate_cumulative_metrics]
[NodeReportGenerator._validate_inputs -> Validates the inputs for node report generation -> dependent functions are None]
[NodeReportGenerator._extract_terminal_nodes -> Recursively finds all terminal nodes in a subtree -> dependent functions are _validate_node_attributes, traverse]
[NodeReportGenerator.traverse -> Traverses the tree to find terminal nodes -> dependent functions are _validate_node_attributes, traverse]
[NodeReportGenerator._validate_node_attributes -> Validates that a node has the required attributes -> dependent functions are None]
[NodeReportGenerator._validate_calculation_results -> Validates the calculation results for edge cases -> dependent functions are None]
[NodeReportGenerator._calculate_lift_metrics -> Calculates lift metrics for a node -> dependent functions are _calculate_lift]
[NodeReportGenerator._get_tree_level_statistics -> Gets overall dataset statistics from the stored tree model data -> dependent functions are _determine_positive_class_from_tree, _safe_divide]
[NodeReportGenerator._determine_positive_class_from_tree -> Determines the positive class from the tree's class counts -> dependent functions are None]
[NodeReportGenerator._determine_positive_class -> Determines the positive class for binary classification -> dependent functions are None]
[NodeReportGenerator._calculate_node_statistics -> Calculates comprehensive statistics for a single node -> dependent functions are _format_node_logic, _generate_node_id, _safe_divide, _calculate_lift_metrics]
[NodeReportGenerator._format_node_logic -> Formats the node's decision path using pre-computed split rules -> dependent functions are _get_child_specific_condition, _build_condition]
[NodeReportGenerator._get_child_specific_condition -> Extracts the specific condition for a child from the parent's split rule -> dependent functions are None]
[NodeReportGenerator._build_condition -> Builds a condition string for a parent-child relationship -> dependent functions are None]
[NodeReportGenerator._create_node_number_mapping -> Creates a node number mapping consistent with the tree visualizer -> dependent functions are _extract_all_nodes]
[NodeReportGenerator._extract_all_nodes -> Extracts all nodes from the decision tree -> dependent functions are traverse]
[NodeReportGenerator._build_alternative_condition -> Provides an alternative method to build a condition string -> dependent functions are None]
[NodeReportGenerator._generate_node_id -> Generates a hierarchical node ID that reflects the tree structure -> dependent functions are None]
[NodeReportGenerator._calculate_cumulative_metrics -> Calculates cumulative metrics for the node report -> dependent functions are None]
[NodeReportGenerator._calculate_lift -> Calculates the lift of a node -> dependent functions are None]
[NodeReportGenerator._safe_divide -> Safely divides two numbers -> dependent functions are None]
[NodeReportGenerator.export_to_excel -> Exports the node report DataFrame to an Excel file -> dependent functions are _format_excel_sheet, timeout_handler]
[NodeReportGenerator.timeout_handler -> Handles timeouts during Excel export -> dependent functions are None]
[NodeReportGenerator._format_excel_sheet -> Applies professional formatting to an Excel sheet -> dependent functions are None]
"""
