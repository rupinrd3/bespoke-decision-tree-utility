...
[NodeAnalyzer.__init__ -> Initializes the node analyzer -> dependent functions are None]
[NodeAnalyzer.get_node_report -> Generates a comprehensive report for a single node -> dependent functions are _get_path_to_node, _calculate_split_quality]
[NodeAnalyzer._get_path_to_node -> Gets the path from the root to this node -> dependent functions are None]
[NodeAnalyzer.analyze_tree -> Analyzes the entire decision tree -> dependent functions are _calculate_balance_factor, _identify_important_nodes]
[NodeAnalyzer._calculate_balance_factor -> Calculates the balance factor of the tree -> dependent functions are None]
[NodeAnalyzer._identify_important_nodes -> Identifies important nodes in the tree -> dependent functions are None]
[NodeAnalyzer.compare_nodes -> Compares two nodes in the tree -> dependent functions are get_node_report]
[NodeAnalyzer.analyze_node_level -> Analyzes nodes at a specific level in the tree -> dependent functions are None]
[NodeAnalyzer.get_node_path_rules -> Gets path rules leading to a node in a human-readable format -> dependent functions are _get_path_to_node]
[NodeAnalyzer.plot_node_class_distribution -> Plots class distribution for a node -> dependent functions are None]
[NodeAnalyzer.plot_node_importance -> Plots node importance based on various metrics -> dependent functions are None]
[NodeAnalyzer._calculate_split_quality -> Calculates enhanced split quality metrics for internal nodes -> dependent functions are None]
[NodeAnalyzer.calculate_terminal_node_metrics -> Calculates performance metrics specifically for terminal nodes -> dependent functions are None]
[NodeAnalyzer.extract_visualization_data -> Extracts data needed for enhanced visualization and reporting -> dependent functions are _get_split_condition]
[NodeAnalyzer._get_split_condition -> Gets the split condition text for an edge -> dependent functions are None]
"""
