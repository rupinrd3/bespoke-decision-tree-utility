...
[TreeNodeContextMenu.__init__ -> Initializes the TreeNodeContextMenu -> dependent functions are setup_styling]
[TreeNodeContextMenu.setup_styling -> Applies modern styling to context menu -> dependent functions are None]
[TreeNodeContextMenu.create_node_menu -> Creates context menu based on node properties -> dependent functions are _add_internal_node_actions, _add_terminal_node_actions, _add_root_node_actions, _add_navigation_actions, _add_utility_actions, _add_destructive_actions]
[TreeNodeContextMenu._add_internal_node_actions -> Adds actions specific to internal nodes -> dependent functions are None]
[TreeNodeContextMenu._add_terminal_node_actions -> Adds actions specific to terminal/leaf nodes -> dependent functions are None]
[TreeNodeContextMenu._add_root_node_actions -> Adds actions specific to root node -> dependent functions are _add_terminal_node_actions, _add_internal_node_actions]
[TreeNodeContextMenu._add_navigation_actions -> Adds tree navigation actions -> dependent functions are None]
[TreeNodeContextMenu._add_utility_actions -> Adds utility actions -> dependent functions are None]
[TreeNodeContextMenu._add_destructive_actions -> Adds destructive actions -> dependent functions are None]
[TreeNodeContextMenu.show_for_node -> Shows context menu for a specific node -> dependent functions are create_node_menu]
[TreeNodeContextMenu.show_at_cursor -> Shows context menu at current mouse cursor position -> dependent functions are show_for_node]
[TreeNodeContextMenu.get_available_actions -> Gets available actions for a node type and state -> dependent functions are None]
"""
