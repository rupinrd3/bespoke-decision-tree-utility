...
[WindowManager.__init__ -> Initializes the window manager -> dependent functions are None]
[WindowManager.get_stacked_widget -> Returns the main stacked widget that holds all the windows -> dependent functions are None]
[WindowManager.register_window -> Registers a new window type with its corresponding widget class -> dependent functions are _create_window_instance]
[WindowManager._create_window_instance -> Creates an instance of a specific window type -> dependent functions are navigate_back]
[WindowManager.show_window -> Switches to a specified window type -> dependent functions are _save_window_state, _create_window_instance, _restore_window_state, _animate_window_transition, can_navigate_back]
[WindowManager.navigate_back -> Navigates to the previously viewed window -> dependent functions are can_navigate_back, show_window]
[WindowManager.navigate_to_workflow -> Navigates directly to the main workflow window -> dependent functions are show_window]
[WindowManager.can_navigate_back -> Checks if it's possible to navigate back -> dependent functions are None]
[WindowManager.get_current_window_type -> Returns the type of the currently active window -> dependent functions are None]
[WindowManager.get_current_window -> Returns the widget of the currently active window -> dependent functions are None]
[WindowManager.get_window_history -> Returns the navigation history -> dependent functions are None]
[WindowManager._save_window_state -> Saves the current state of a window -> dependent functions are None]
[WindowManager._restore_window_state -> Restores the saved state of a window -> dependent functions are None]
[WindowManager._animate_window_transition -> Animates the transition between windows -> dependent functions are None]
[WindowManager.clear_window_cache -> Clears the cached window instances -> dependent functions are clear_window_cache]
[WindowManager.get_window_count -> Returns the number of cached window instances -> dependent functions are None]
[WindowManager.cleanup -> Cleans up resources used by the window manager -> dependent functions are _save_window_state]
[BreadcrumbWidget.__init__ -> Initializes the breadcrumb navigation widget -> dependent functions are setup_ui]
[BreadcrumbWidget.setup_ui -> Sets up the user interface for the breadcrumb widget -> dependent functions are None]
[BreadcrumbWidget.update_breadcrumb -> Updates the breadcrumb display to reflect the current window -> dependent functions are None]
[BreadcrumbWidget.set_window_titles -> Sets custom titles for the windows in the breadcrumb -> dependent functions are None]
"""
