# utility/ui/dialogs/text_options_dialog.py
#!/usr/bin/env python
# -*- coding: utf-8 -*-

"""
Text File Import Options Dialog for Bespoke Utility
"""
...
def _decode_escape(value: str) -> str:
    """Decode escape sequences (e.g., \t) entered by the user."""
    if not value:
        return value
    try:
...
class TextImportOptionsDialog(QDialog):
    """Dialog for configuring Text file import options."""

    def __init__(self, default_encoding='utf-8', default_delimiter='\t', parent=None):
        super().__init__(parent)
        self.setWindowTitle("Text File Import Options")
        ...
        self.delimiter_edit = QLineEdit(default_delimiter if default_delimiter != '\t' else '\\t')
        self.delimiter_edit.setToolTip("Use '\\t' for tab delimiter.")
        ...
        self.date_columns_edit.setPlaceholderText("Comma separated, e.g. event_date,settle_date")
        ...
        date_hint = QLabel("Provide columns to parse as dates; format is optional.")
    ...
    def accept_options(self):
        ...
        logger.debug(f"Text Import options set: {self.options}")
        self.accept()
