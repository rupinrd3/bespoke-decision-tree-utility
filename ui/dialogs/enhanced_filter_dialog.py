...
[EnhancedFilterDialog.__init__ -> Initializes the EnhancedFilterDialog -> dependent functions are setup_ui, _populate_fields, _load_existing_conditions]
[EnhancedFilterDialog.setup_ui -> Sets up the user interface -> dependent functions are _create_condition_builder, _create_condition_list, _create_preview_area, _create_button_bar]
[EnhancedFilterDialog._create_condition_builder -> Creates the condition builder section -> dependent functions are _update_operator_and_value_widgets, _add_condition]
[EnhancedFilterDialog._create_condition_list -> Creates the condition list section -> dependent functions are _remove_selected_condition, _clear_all_conditions]
[EnhancedFilterDialog._create_preview_area -> Creates the preview area -> dependent functions are None]
[EnhancedFilterDialog._create_button_bar -> Creates the button bar -> dependent functions are apply_filters, reject]
[EnhancedFilterDialog._populate_fields -> Populates the fields in the dialog -> dependent functions are _update_operator_and_value_widgets]
[EnhancedFilterDialog._load_existing_conditions -> Loads existing conditions from the filter node -> dependent functions are _add_condition_to_list]
[EnhancedFilterDialog._update_operator_and_value_widgets -> Updates the operator and value widgets based on the selected column -> dependent functions are None]
[EnhancedFilterDialog._add_condition -> Adds a new condition -> dependent functions are _add_condition_to_list, _update_preview]
[EnhancedFilterDialog._add_condition_to_list -> Adds a condition to the list widget -> dependent functions are None]
[EnhancedFilterDialog._remove_selected_condition -> Removes the selected condition -> dependent functions are _update_preview]
[EnhancedFilterDialog._clear_all_conditions -> Clears all conditions -> dependent functions are _update_preview]
[EnhancedFilterDialog._update_preview -> Updates the data preview -> dependent functions are None]
[EnhancedFilterDialog.apply_filters -> Applies the filters -> dependent functions are None]
"""
