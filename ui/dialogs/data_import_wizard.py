...
[DataImportWizard.__init__ -> Initializes the DataImportWizard -> dependent functions are on_page_changed]
[DataImportWizard.on_page_changed -> Handles page changes -> dependent functions are None]
[DataImportWizard.accept -> Handles wizard completion -> dependent functions are None]
[SourceSelectionPage.__init__ -> Initializes the SourceSelectionPage -> dependent functions are None]
[SourceSelectionPage.nextId -> Determines the next page ID -> dependent functions are None]
[SourceSelectionPage.initializePage -> Initializes the page -> dependent functions are None]
[SourceSelectionPage.validatePage -> Validates the page -> dependent functions are None]
[FileOptionsPage.__init__ -> Initializes the FileOptionsPage -> dependent functions are browse_file, _create_csv_options, _create_excel_options, _create_text_options]
[FileOptionsPage.browse_file -> Browses for a file -> dependent functions are None]
[FileOptionsPage._create_csv_options -> Creates CSV import options -> dependent functions are None]
[FileOptionsPage._create_excel_options -> Creates Excel import options -> dependent functions are None]
[FileOptionsPage._create_text_options -> Creates text import options -> dependent functions are None]
[FileOptionsPage.isComplete -> Checks if the page is complete -> dependent functions are None]
[FileOptionsPage.nextId -> Goes to the data preview page -> dependent functions are None]
[DatabaseOptionsPage.__init__ -> Initializes the DatabaseOptionsPage -> dependent functions are None]
[DatabaseOptionsPage.isComplete -> Checks if the page is complete -> dependent functions are None]
[DatabaseOptionsPage.nextId -> Goes to the data preview page -> dependent functions are None]
[CloudOptionsPage.__init__ -> Initializes the CloudOptionsPage -> dependent functions are None]
[CloudOptionsPage.isComplete -> Checks if the page is complete -> dependent functions are None]
[CloudOptionsPage.nextId -> Goes to the data preview page -> dependent functions are None]
[DataPreviewPage.__init__ -> Initializes the DataPreviewPage -> dependent functions are None]
[DataPreviewPage.initializePage -> Loads and previews the data -> dependent functions are load_data]
[DataPreviewPage.load_data -> Loads data based on previous selections -> dependent functions are _load_from_file, _load_from_database, _load_from_cloud, populate_preview_table]
[DataPreviewPage._load_from_file -> Loads data from a local file -> dependent functions are None]
[DataPreviewPage._load_from_database -> Loads data from a database -> dependent functions are None]
[DataPreviewPage._load_from_cloud -> Loads data from cloud storage -> dependent functions are None]
[DataPreviewPage.populate_preview_table -> Populates the preview table with loaded data -> dependent functions are None]
[DataPreviewPage.isComplete -> Checks if the page is complete -> dependent functions are None]
[DataPreviewPage.nextId -> Goes to the finalize page -> dependent functions are None]
[FinalizePage.__init__ -> Initializes the FinalizePage -> dependent functions are None]
[FinalizePage.initializePage -> Initializes the page -> dependent functions are None]
[FinalizePage.validatePage -> Validates the page -> dependent functions are None]
"""
...
import pandas as pd
import numpy as np
from PyQt5.QtCore import Qt, pyqtSignal, QThread, QTimer, QRegularExpression
from PyQt5.QtGui import QFont, QPixmap, QIcon, QRegularExpressionValidator
from PyQt5.QtWidgets import (
    ...
    QFileDialog, QSlider, QSpacerItem, QSizePolicy, QListWidgetItem
)
...
def _display_char(value: Optional[str], fallback: str) -> str:
    """Render control characters so they are visible in line edits/labels"""
    if value in (None, ''):
        return fallback
    return _SPECIAL_CHAR_DISPLAY.get(value, value)


def _decode_char_input(value: str) -> str:
    """Decode common escape sequences (e.g., \t) entered in line edits"""
    if not value:
        return value
    try:
        return bytes(value, 'utf-8').decode('unicode_escape')
    except Exception as exc:
        logger.debug(f"Could not decode text field '{value}': {exc}")
        return value
...
class IntroPage(QWizardPage):
    ...
    def __init__(self):
        ...
        <ul>
        <li>CSV and other delimited text files</li>
        <li>Excel workbooks (.xlsx, .xls)</li>
        <li>Columnar formats (Parquet, Feather)</li>
        <li>Serialized pandas data (.pkl, .pickle)</li>
        ...
        <p><b>Features:</b></p>
        <ul>
        <li>Enhanced file detection with format-specific options</li>
        <li>Interactive data preview and validation</li>
        <li>Automatic and manual data type detection</li>
        <li>Custom date parsing and missing value strategies</li>
        <li>Column selection, mapping, and transformation</li>
        </ul>
        """)
...
class SourceSelectionPage(QWizardPage):
    ...
    def __init__(self, wizard: DataImportWizard):
        ...
        self.file_radio = QRadioButton("File (CSV, Excel, Text, Parquet, Feather, Pickle)")
    ...
    def browse_file(self):
        ...
        file_path, _ = QFileDialog.getOpenFileName(
            ...
            "Select Data File",
            "",
            "All Supported (*.csv *.xlsx *.xls *.txt *.tsv *.parquet *.pqt *.feather *.ft *.pkl *.pickle);;"
            "CSV Files (*.csv);;"
            "Excel Files (*.xlsx *.xls);;"
            "Text Files (*.txt *.tsv);;"
            "Parquet Files (*.parquet *.pqt);;"
        ...
        if file_path:
            ...
            if ext == '.csv':
                ...
                self.wizard.import_config['file_format'] = 'parquet'
            elif ext in ['.feather', '.ft']:
                self.wizard.import_config['file_format'] = 'feather'
            elif ext in ['.pkl', '.pickle']:
                self.wizard.import_config['file_format'] = 'pickle'
...
class FormatConfigurationPage(QWizardPage):
    ...
    def __init__(self, wizard: DataImportWizard):
        ...
        self.format_combo.addItems(['csv', 'excel', 'text', 'parquet', 'feather', 'pickle'])
        ...
        self._char_validator = QRegularExpressionValidator(char_pattern, self)
        ...
        self.quote_char_edit.setValidator(self._char_validator)
        text_layout.addRow("Quote Character:", self.quote_char_edit)
        
        self.escape_char_edit = QLineEdit("")
        self.escape_char_edit.setMaxLength(2)
        self.escape_char_edit.setPlaceholderText("e.g. \\")
        self.escape_char_edit.setValidator(self._char_validator)
        text_layout.addRow("Escape Character:", self.escape_char_edit)
        ...
        text_layout.addRow("Skip Rows (Top):", self.skip_spin)
        
        self.skip_footer_spin = QSpinBox()
        self.skip_footer_spin.setRange(0, 1000)
        text_layout.addRow("Skip Rows (Bottom):", self.skip_footer_spin)
        
        self.decimal_edit = QLineEdit(".")
        self.decimal_edit.setMaxLength(2)
        self.decimal_edit.setPlaceholderText("Default: .")
        self.decimal_edit.setValidator(self._char_validator)
        text_layout.addRow("Decimal Separator:", self.decimal_edit)

        self.thousands_edit = QLineEdit("")
        self.thousands_edit.setMaxLength(2)
        self.thousands_edit.setPlaceholderText("Optional (e.g. ,)")
        self.thousands_edit.setValidator(self._char_validator)
        text_layout.addRow("Thousands Separator:", self.thousands_edit)
        ...
        self.datetime_group = QGroupBox("Date/Time Options")
        ...
        datetime_layout.addRow("Date Format:", self.date_format_edit)
        
        date_hint = QLabel("Specify a format to parse date columns. Columns can be selected on the preview page.")
        date_hint.setWordWrap(True)
        datetime_layout.addRow("", date_hint)
    ...
    def initializePage(self):
        ...
        delimiter_display = _display_char(config.get('delimiter', ','), ',')
        self.encoding_combo.setCurrentText(config.get('encoding', 'utf-8'))
        self.delimiter_edit.setText(delimiter_display)
        self.quote_char_edit.setText(_display_char(config.get('quotechar'), '"'))
        self.escape_char_edit.setText(_display_char(config.get('escapechar'), ''))
        self.header_spin.setValue(config.get('header_row', 0))
        self.skip_spin.setValue(config.get('skip_rows', 0))
        self.skip_footer_spin.setValue(config.get('skip_footer', 0))
        self.decimal_edit.setText(_display_char(config.get('decimal', '.'), '.'))
        self.thousands_edit.setText(_display_char(config.get('thousands'), ''))
        self.date_format_edit.setText(config.get('date_format', ''))
    ...
    def update_format_options(self):
        ...
        if format_type in ['csv', 'text']:
            ...
            self.text_group.setVisible(False)
            self.excel_group.setVisible(False)
        self.datetime_group.setVisible(True)
    ...
    def validatePage(self):
        ...
        self.wizard.import_config['delimiter'] = _decode_char_input(self.delimiter_edit.text())
        self.wizard.import_config['header_row'] = self.header_spin.value()
        self.wizard.import_config['skip_rows'] = self.skip_spin.value()
        self.wizard.import_config['skip_footer'] = self.skip_footer_spin.value()
        
        quote_val = self.quote_char_edit.text()
        self.wizard.import_config['quotechar'] = quote_val if quote_val else None
        
        escape_val = self.escape_char_edit.text()
        self.wizard.import_config['escapechar'] = escape_val if escape_val else ''
        
        decimal_val = _decode_char_input(self.decimal_edit.text()) or '.'
        thousands_val = _decode_char_input(self.thousands_edit.text())

        if decimal_val and thousands_val and decimal_val == thousands_val:
            QMessageBox.warning(self, "Validation Error", "Decimal and thousands separators must differ.")
            return False

        self.wizard.import_config['decimal'] = decimal_val
        self.wizard.import_config['thousands'] = thousands_val or ''
        
        self.wizard.import_config['date_format'] = self.date_format_edit.text().strip()
...
class DataPreviewPage(QWizardPage):
    ...
    def __init__(self, wizard: DataImportWizard):
        ...
        self.column_group = QGroupBox("Columns to Import")
        column_layout = QVBoxLayout()
        self.column_list = QListWidget()
        self.column_list.setAlternatingRowColors(True)
        self.column_list.setSelectionMode(QListWidget.NoSelection)
        self.column_list.itemChanged.connect(self.on_column_item_changed)
        column_layout.addWidget(self.column_list)
        
        column_button_layout = QHBoxLayout()
        self.select_all_columns_btn = QPushButton("Select All")
        self.select_all_columns_btn.clicked.connect(self.select_all_columns)
        self.clear_columns_btn = QPushButton("Clear All")
        self.clear_columns_btn.clicked.connect(self.clear_column_selection)
        column_button_layout.addWidget(self.select_all_columns_btn)
        column_button_layout.addWidget(self.clear_columns_btn)
        column_button_layout.addStretch()
        column_layout.addLayout(column_button_layout)
        self.column_group.setLayout(column_layout)
        right_layout.addWidget(self.column_group)
        
        self.date_group = QGroupBox("Date/Time Parsing")
        date_layout = QVBoxLayout()
        self.date_list = QListWidget()
        self.date_list.setAlternatingRowColors(True)
        self.date_list.setSelectionMode(QListWidget.NoSelection)
        self.date_list.itemChanged.connect(self.on_date_item_changed)
        date_layout.addWidget(self.date_list)
        
        date_button_layout = QHBoxLayout()
        self.select_all_dates_btn = QPushButton("Mark All")
        self.select_all_dates_btn.clicked.connect(self.select_all_dates)
        self.clear_dates_btn = QPushButton("Clear")
        self.clear_dates_btn.clicked.connect(self.clear_date_selection)
        date_button_layout.addWidget(self.select_all_dates_btn)
        date_button_layout.addWidget(self.clear_dates_btn)
        date_button_layout.addStretch()
        date_layout.addLayout(date_button_layout)
        
        date_format_label = QLabel("Custom Date Format:")
        self.date_format_line = QLineEdit(self.wizard.import_config.get('date_format', ''))
        self.date_format_line.setPlaceholderText("e.g. %Y-%m-%d")
        self.date_format_line.textChanged.connect(self.on_date_format_changed)
        date_layout.addWidget(date_format_label)
        date_layout.addWidget(self.date_format_line)
        
        date_hint = QLabel("Checked columns will be parsed as dates using the format above (if provided).")
        date_hint.setWordWrap(True)
        date_layout.addWidget(date_hint)
        
        self.date_group.setLayout(date_layout)
        right_layout.addWidget(self.date_group)
        ...
        self._updating_column_checks = False
        self._updating_date_checks = False
    ...
    def initializePage(self):
        ...
        self.date_format_line.blockSignals(True)
        self.date_format_line.setText(self.wizard.import_config.get('date_format', ''))
        self.date_format_line.blockSignals(False)
    ...
    def refresh_preview(self):
        ...
        try:
            ...
            format_type = self.wizard.import_config.get('file_format', 'csv')
            date_columns = self.wizard.import_config.get('date_columns', [])
            date_format = self.wizard.import_config.get('date_format', '')
            df = None
            metadata = {}
            ...
            if format_type in ['csv', 'text']:
                kwargs = {
                    ...
                    'quotechar': self.wizard.import_config.get('quotechar'),
                    'escapechar': self.wizard.import_config.get('escapechar'),
                    ...
                    'skipfooter': self.wizard.import_config.get('skip_footer', 0),
                    'decimal': self.wizard.import_config.get('decimal', '.'),
                    'thousands': self.wizard.import_config.get('thousands', ''),
                    'na_values': self.wizard.import_config.get('missing_values', []),
                ...
                kwargs = {
                    'sheet_name': self.wizard.import_config.get('sheet_name', 0),
                    'skiprows': self.wizard.import_config.get('skip_rows', 0),
                    'skipfooter': self.wizard.import_config.get('skip_footer', 0),
                    'nrows': max_preview_rows,
                    'na_values': self.wizard.import_config.get('missing_values', []),
    ...
    def update_preview_table(self, df: pd.DataFrame):
        ...
        try:
            display_df = df
            selected_columns = self.wizard.import_config.get('selected_columns')
            if selected_columns:
                existing_columns = [col for col in selected_columns if col in display_df.columns]
                if existing_columns:
                    display_df = display_df.loc[:, existing_columns]

            max_rows = min(len(display_df), 500)  # Limit to 500 rows max for performance
            max_cols = min(len(display_df.columns), 50)  # Limit to 50 columns max
            
            display_df = display_df.iloc[:max_rows, :max_cols]
    ...
    def _get_checked_columns(self, list_widget: QListWidget) -> List[str]:
        """Return checked column names from a QListWidget"""
        return [
    ...
    def populate_column_lists(self, df: pd.DataFrame):
        """Populate the column selection controls"""
        all_columns = list(df.columns)
        self.wizard.import_config['column_names'] = all_columns
        ...
        for col in all_columns:
            ...
            checked = selected_columns is None or col in selected_columns
        ...
        if selected_columns is None or len(selected_columns) == len(all_columns):
            self._set_selected_columns(None)
        else:
    ...
    def populate_date_list(self):
        """Populate the date column checklist"""
        all_columns = self.wizard.import_config.get('column_names', [])
        selected_columns = self.wizard.import_config.get('selected_columns')
        if selected_columns is None or not selected_columns:
            display_columns = all_columns
        else:
            display_columns = [col for col in all_columns if col in selected_columns]

        current_dates = self.wizard.import_config.get('date_columns', [])
        filtered_dates = [col for col in current_dates if col in display_columns]
        if filtered_dates != current_dates:
            self.wizard.import_config['date_columns'] = filtered_dates
        ...
        for col in display_columns:
            item = QListWidgetItem(str(col))
            item.setFlags(item.flags() | Qt.ItemIsUserCheckable)
            item.setCheckState(Qt.Checked if col in self.wizard.import_config.get('date_columns', []) else Qt.Unchecked)
            self.date_list.addItem(item)

        self.date_list.blockSignals(False)
        self._updating_date_checks = False

    def on_column_item_changed(self, item: QListWidgetItem):
        """Handle column import selection changes"""
        if self._updating_column_checks:
            return

        selected = self._get_checked_columns(self.column_list)
        if not selected:
            QMessageBox.warning(self, "Column Selection", "At least one column must remain selected.")
            self._updating_column_checks = True
            self.column_list.blockSignals(True)
        ...
        if len(selected) == self.column_list.count():
            self._set_selected_columns(None)
        else:
            self._set_selected_columns(selected)

        current_dates = self.wizard.import_config.get('date_columns', [])
        if self.wizard.import_config['selected_columns'] is None:
            allowed = set(self.wizard.import_config.get('column_names', []))
        else:
            allowed = set(selected)
        filtered_dates = [col for col in current_dates if col in allowed]
        if filtered_dates != current_dates:
            self.wizard.import_config['date_columns'] = filtered_dates

        self.populate_date_list()
        preview_df = self.wizard.import_config.get('preview_data')
    ...
    def clear_column_selection(self):
        """Clear selection while keeping at least one column checked"""
        if self.column_list.count() == 0:
            return
        self._updating_column_checks = True
        self.column_list.blockSignals(True)
        for i in range(self.column_list.count()):
            self.column_list.item(i).setCheckState(Qt.Checked if i == 0 else Qt.Unchecked)
        self.column_list.blockSignals(False)
        self._updating_column_checks = False

        if self.column_list.count() == 1:
            self._set_selected_columns(None)
        else:
            self._set_selected_columns([self.column_list.item(0).text()])
        self.populate_date_list()
        preview_df = self.wizard.import_config.get('preview_data')
        if preview_df is not None:
            self.update_preview_table(preview_df)

    def on_date_item_changed(self, item: QListWidgetItem):
        """Handle date column toggles"""
        if self._updating_date_checks:
            return
        self.wizard.import_config['date_columns'] = self._get_checked_columns(self.date_list)

    def select_all_dates(self):
        """Mark all displayed columns as dates"""
        ...
        self.date_list.blockSignals(False)
        self._updating_date_checks = False
        self.wizard.import_config['date_columns'] = [
            self.date_list.item(i).text() for i in range(self.date_list.count())
        ]

    def clear_date_selection(self):
        """Unmark all date columns"""
        self._updating_date_checks = True
        self.date_list.blockSignals(True)
        for i in range(self.date_list.count()):
            self.date_list.item(i).setCheckState(Qt.Unchecked)
        self.date_list.blockSignals(False)
        self._updating_date_checks = False
        self.wizard.import_config['date_columns'] = []

    def on_date_format_changed(self, text: str):
        """Persist custom date format text"""
        self.wizard.import_config['date_format'] = text.strip()

    def _set_selected_columns(self, columns: Optional[List[str]]):
        """Persist selected columns and keep usecols in sync"""
        if not columns:
            self.wizard.import_config['selected_columns'] = None
            if 'usecols' in self.wizard.import_config:
                self.wizard.import_config.pop('usecols')
        else:
            self.wizard.import_config['selected_columns'] = columns
            self.wizard.import_config['usecols'] = columns
...
class SummaryPage(QWizardPage):
    ...
    def update_summary(self):
        ...
        config = self.wizard.import_config
        delimiter_display = _display_char(config.get('delimiter', ','), ',')
        quote_display = _display_char(config.get('quotechar'), 'default')
        escape_display = _display_char(config.get('escapechar'), 'None')
        thousands_display = _display_char(config.get('thousands'), 'None')
        selected_columns_display = ', '.join(config.get('selected_columns')) if config.get('selected_columns') else 'All columns'
        date_columns_display = ', '.join(config.get('date_columns', [])) if config.get('date_columns') else 'None'
        date_format_display = config.get('date_format', '')
        ...
        <li><b>Delimiter:</b> "{delimiter_display}"</li>
        <li><b>Header Row:</b> {config.get('header_row', 0)}</li>
        <li><b>Skip Rows (Top):</b> {config.get('skip_rows', 0)}</li>
        <li><b>Skip Rows (Bottom):</b> {config.get('skip_footer', 0)}</li>
        <li><b>Quote Character:</b> {quote_display}</li>
        <li><b>Escape Character:</b> {escape_display}</li>
        <li><b>Decimal Separator:</b> {config.get('decimal', '.')}</li>
        <li><b>Thousands Separator:</b> {thousands_display}</li>
        </ul>

        <h4>Column Selection:</h4>
        <ul>
        <li><b>Selected Columns:</b> {selected_columns_display}</li>
        <li><b>Date Columns:</b> {date_columns_display}
            {(' (Format: ' + date_format_display + ')') if config.get('date_columns') and date_format_display else ''}</li>
        </ul>
