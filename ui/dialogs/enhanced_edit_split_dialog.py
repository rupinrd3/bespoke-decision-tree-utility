    ...
    def update_all_bin_stats(self):
        ...
        try:
            ...
            if node_data is None or self.current_feature not in node_data.columns:
                ...
                return
            feature_series = node_data[self.current_feature]
            normalized_feature = None
            if not pd.api.types.is_numeric_dtype(feature_series):
                normalized_feature = feature_series.apply(
                ...
                try:
                    ...
                    if pd.api.types.is_numeric_dtype(feature_series):
                        if i == len(self.bins) - 1:  # Last bin includes max value
                            mask = (feature_series >= min_val) & (feature_series <= max_val)
                        else:
                            mask = (feature_series >= min_val) & (feature_series < max_val)
                        ...
                        mask = normalized_feature == min_val
