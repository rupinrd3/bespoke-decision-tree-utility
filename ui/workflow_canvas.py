...
# [NodeConfigDialog.get_node_config -> Gets the updated node configuration -> dependent functions are None]
"""
...
class WorkflowNode(QGraphicsRectItem):
    ...
    def __init__(self, node_id: str, node_type: str, title: str, parent=None):
        ...
        self.text_item.setPos(12, 12)
        self._configure_text_item()
    ...
    def _configure_text_item(self):
        """Configure shared font and layout traits for cross-platform rendering."""
        base_font = QFont("Segoe UI", 8)
        base_font.setStyleStrategy(QFont.PreferAntialias)
        self.text_item.document().setDefaultFont(base_font)
        self.text_item.setTextWidth(self.width - 24)
    
    def _set_text_lines(self, content_lines):
        """Render the provided lines inside the text item with consistent styling."""
        html = (
    ...
    def _get_node_icon(self) -> str:
        ...
        icons = {
            NODE_TYPE_DATASET: "[DS]",
            NODE_TYPE_MODEL: "[ML]",
            NODE_TYPE_FILTER: "[FL]",
            NODE_TYPE_TRANSFORM: "[TR]",
            NODE_TYPE_EVALUATION: "[EV]",
            NODE_TYPE_VISUALIZATION: "[VZ]",
            NODE_TYPE_EXPORT: "[EX]"
        }
    ...
    def _update_text_content(self):
        ...
        self._set_text_lines(content_lines)
...
class DatasetNode(WorkflowNode):
    ...
    def update_dataset_info(self, rows: int = None, cols: int = None):
        ...
        if rows is not None and cols is not None:
            content_lines.append(f"Rows: {rows}")
            content_lines.append(f"Columns: {cols}")
...
class FilterNode(WorkflowNode):
    ...
    def update_filter_info(self):
        ...
        if self.conditions:
            content_lines.append(f"Conditions: {len(self.conditions)}")
            first_condition = self.conditions[0]
            content_lines.append(f"• {first_condition['column']} {first_condition['operator']} {first_condition['value']}")
            if len(self.conditions) > 1:
                content_lines.append(f"+ {len(self.conditions)-1} more…")
        else:
            content_lines.append("No conditions set")
        
        if self.filtered_row_count > 0:
            result_text = f"Result: {self.filtered_row_count:,} rows"
            if self.original_row_count > 0:
                pct = (self.filtered_row_count / self.original_row_count) * 100
                result_text += f" ({pct:.1f}%)"
            content_lines.append(result_text)
...
class TransformNode(WorkflowNode):
    ...
    def update_transform_info(self):
        ...
        if self.transformations:
            content_lines.append(f"Transforms: {len(self.transformations)}")
            first_transform = self.transformations[0]
            content_lines.append(f"• {first_transform['type']}")
            content_lines.append(f"→ {first_transform['target_column']}")
            if len(self.transformations) > 1:
                content_lines.append(f"+ {len(self.transformations)-1} more…")
        else:
            content_lines.append("No transformations set")
        
        if self.transformed_column_count > 0:
            added_cols = self.transformed_column_count - self.original_column_count
            if added_cols > 0:
                content_lines.append(f"Added: {added_cols} columns")
            else:
                content_lines.append(f"Columns: {self.transformed_column_count}")
...
class EvaluationNode(WorkflowNode):
    ...
    def update_evaluation_info(self):
        ...
        self._configure_text_item()
        content_lines = [f"<b>{self.title}</b>"]

        if self.metrics:
            content_lines.append(f"Metrics: {', '.join(self.metrics)}")
        else:
            content_lines.append("No metrics selected")
        
        if self.cross_validation:
            content_lines.append(f"CV: {self.n_folds} folds")
...
class VisualizationNode(WorkflowNode):
    ...
    def update_visualization_info(self):
        ...
        content_lines = [
            f"<b>{self.title}</b>",
            f"Type: {self.visualization_type.capitalize()}",
        ]
        self._set_text_lines(content_lines)
...
class ExportNode(WorkflowNode):
    ...
    def update_export_info(self):
        ...
        content_lines = [
            f"<b>{self.title}</b>",
            f"Type: {self.export_type.capitalize()}",
        ]

        if self.export_path:
            content_lines.append(f"Path: …{self.export_path[-20:]}")
...
class WorkflowScene(QGraphicsScene):
    ...
    def set_config(self, config: Dict[str, Any]):
        ...
        for conn_id, conn_config in config.get('connections', {}).items():
            ...
            self._create_connection_line(connection, source_node, target_node, source_port_obj, target_port_obj)
            ...
            logger.info(
                f"Successfully restored connection {conn_id}: "
                f"{source_node_id}[{source_port_idx}] -> {target_node_id}[{target_port_idx}]"
            )
...
class NodeConfigDialog(QDialog):
    ...
    def accept(self):
        ...
        if isinstance(self.node, WorkflowNode):
            self.node._update_text_content()
...
class WorkflowCanvas(QWidget):
    ...
    def get_upstream_node_ids(self, node_id: str, connection_type: Optional[str] = None) -> List[str]:
        """
        Get IDs for all source nodes connected to the specified node.
        
        Args:
            node_id: Target node ID.
            connection_type: Optional connection type filter (data, model, result).
        
        Returns:
            List of source node IDs feeding the node.
        """
        if not node_id:
            return []
        ...
        for conn in self.scene.connections.values():
            if not hasattr(conn, 'target_node_id'):
                continue
            if conn.target_node_id != node_id:
                continue
            if connection_type and getattr(conn, 'connection_type', None) != connection_type:
                continue
            upstream_ids.append(conn.source_node_id)
