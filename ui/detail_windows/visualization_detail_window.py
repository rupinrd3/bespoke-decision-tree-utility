...
class VisualizationDetailWindow(BaseDetailWindow):
    ...
    def create_model_overview_tab(self):
        ...
        summary_layout.addWidget(self.dataset_info_label, 2, 0, 1, 2)
    ...
    def load_data(self, data: Dict[str, Any]):
        ...
        try:
            ...
            node_name = data.get('name') or f"Visualization {node_id}"
            self.dataset_label = data.get('dataset_label')
            
            if self.dataset_label:
                self.set_window_title(f"{node_name} · {self.dataset_label}")
            else:
                self.set_window_title(node_name)
            ...
            if self.dataset_label:
                logger.info(f"Dataset label: {self.dataset_label}")
    ...
    def populate_model_overview(self):
        ...
        try:
            ...
            if self.dataset is not None:
                rows, cols = getattr(self.dataset, 'shape', (0, 0))
                dataset_name = self.dataset_label or "Workflow Dataset"
                dataset_desc = f"Dataset: {dataset_name} ({rows:,} rows, {cols:,} cols)"
            elif self.dataset_label:
                dataset_desc = f"Dataset: {self.dataset_label}"
            self._safe_set_text('dataset_info_label', dataset_desc)
