    ...
    def _calculate_dimensions(self) -> Dict[str, int]:
        ...
        if self.node.class_counts:
            for class_label, count in self.node.class_counts.items():
                ...
                required_width = class_width + count_width + perc_width + 70  # Increased spacing to avoid overlap
        ...
        if self.node.samples:
            ...
            total_required_width = total_label_width + count_width + perc_width + 50
            max_width = max(max_width, total_required_width)
            total_height += line_height + 4  # Extra spacing before total
        
        total_height += 10
        
        final_width = max(int(max_width * 1.15), 216)
        return {'width': final_width, 'height': total_height}
...
class EnhancedTreeEdgeItem(QGraphicsPathItem):
    ...
    def _get_bin_value_text(self) -> str:
        ...
        if self.parent_node.split_type == 'numeric':
            ...
            if hasattr(self.parent_node, 'split_bin_labels') and self.parent_node.split_bin_labels:
                if self.child_index < len(self.parent_node.split_bin_labels):
                    return self.parent_node.split_bin_labels[self.child_index]
            if hasattr(self.child_node, 'bin_display_label') and getattr(self.child_node, 'bin_display_label'):
                return self.child_node.bin_display_label
